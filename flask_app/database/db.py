...
def get_avisos_por_mes_tipo():
	conn = get_conn()
	cursor = conn.cursor(pymysql.cursors.DictCursor)
	cursor.execute(QUERY_DICT["get_avisos_por_mes_y_tipo"])
	avisos = cursor.fetchall()
	cursor.close()
	conn.close()
	return avisos

def add_comentario(nombre, contenido, fecha, aviso_id):
	conn = get_conn()
	cursor = conn.cursor()
	cursor.execute(QUERY_DICT["add_comentario"], (nombre, contenido, fecha, aviso_id))
	conn.commit()
	cursor.close()
	conn.close()

def get_comments_by_notice_id(aviso_id):
	conn = get_conn()
	cursor = conn.cursor(pymysql.cursors.DictCursor)
	cursor.execute(QUERY_DICT["get_comments_by_notice_id"], (aviso_id,))
