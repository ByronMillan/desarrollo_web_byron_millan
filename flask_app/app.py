...
from utils.validations import validate_form, get_contactos, validate_comentario
...
def post_com():
    ...
    errores = validate_comentario(nombre, contenido)
    if errores:
        return jsonify({"success": False, "errores": errores}), 400
    
    fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    db.add_comentario(nombre, contenido, fecha, aviso_id)
...
@app.route("/get-comentarios/<int:aviso_id>", methods=["GET"])
def get_comentarios(aviso_id):
    comentarios = db.get_comments_by_notice_id(aviso_id)
...
@cross_origin(origins=["http://127.0.0.1:5000", "http://localhost:5000"], supports_credentials=True)
def get_stats_data():

    avisos_dia = db.get_avisos_por_dia()
    avisos_tipo = db.get_avisos_por_tipo()
    avisos_mes_tipo = db.get_avisos_por_mes_tipo()

    for row in avisos_dia:
        row["fecha"] = row["fecha"].strftime("%Y-%m-%d")

    return jsonify({
        "avisos_por_dia": avisos_dia,
        "avisos_por_tipo": avisos_tipo,
        "avisos_por_mes_tipo": avisos_mes_tipo
    })
